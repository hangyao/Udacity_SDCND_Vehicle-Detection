...
# Modified this function to return a normalized histgram
def color_hist(img, nbins=32):
    # Compute the histogram of the color channels separately
    channel1_hist, bin_edges = np.histogram(img[:,:,0], bins=nbins, density=True)
    channel1_hist = channel1_hist * np.diff(bin_edges)
    channel2_hist, bin_edges = np.histogram(img[:,:,1], bins=nbins, density=True)
    channel2_hist = channel2_hist * np.diff(bin_edges)
    channel3_hist, bin_edges = np.histogram(img[:,:,2], bins=nbins, density=True)
    channel3_hist = channel3_hist * np.diff(bin_edges)
...
def slide_window(img, x_start_stop=[None, None], y_start_stop=[500, 720],
